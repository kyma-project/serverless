...
- Functions' SecurityContext can be configured using the `.spec.podSecurityContext` and the `.spec.containerSecurityContext` fields ( read more about [custom resource parameters](./resources/06-20-serverless-cr.md#custom-resource-parameters) ). In such case you are responsible for setting up production grade policies following the [Pod Security Standards](https://kubernetes.io/docs/concepts/security/pod-security-standards/).
