...
- `lib/tracer.cjs` — OpenTelemetry tracer setup (NodeTracerProvider, OTLP exporter, Express+HTTP instrumentation)
- `lib/metrics.cjs` — Prometheus metrics via OpenTelemetry SDK (PrometheusExporter)
- `lib/helper.cjs` — Request timeout middleware, graceful shutdown, error handling utilities (CommonJS)

## Key Design Decisions

- **ESM throughout**: `server.mjs` and `sdk/index.mjs` are ESM. `lib/*.cjs` files remain CommonJS (they predate the rework and are internal).
