...
func TestBuildResources(t *testing.T) {
	...
	t.Run("build resources for function with specified app name", func(t *testing.T) {
		...
		fmt.Println(files)
		require.Equal(t, "k8s/service.yaml", files[0].Name)
		requireEqualBase64Objects(t, fixTestServiceWithAppName(), files[0].Data)
		require.Equal(t, "k8s/deployment.yaml", files[1].Name)
		requireEqualBase64Objects(t, fixDeploymentWithAppName(), files[1].Data)
	})
}
...
func fixTestServiceWithAppName() string {
	return `apiVersion: v1
kind: Service
metadata:
  creationTimestamp: null
...
spec:
  ...
  - name: http
    ...
    targetPort: 8080
  selector:
    app.kubernetes.io/instance: test-app-name
    serverless.kyma-project.io/resource: deployment
status:
  loadBalancer: {}
`
...
func fixDeploymentWithAppName() string {
	return `apiVersion: apps/v1
kind: Deployment
metadata:
  creationTimestamp: null
